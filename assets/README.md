...
## V1.2 GitHub Pages 预览资源说明

V1.2 没有新增图片资源，也没有引入远程图片、远程字体或远程脚本。当前图片继续保持本地化引用，适合 GitHub Pages 项目页路径 `/youmu/` 下加载。

本轮检查图片体积后未执行压缩。原因是当前最大图片约 654KB，仍处于预览阶段可接受范围；二维码、木纹和首屏图片不宜为了轻微体积收益过度压缩。正式上线前如替换真实图片，再按最终素材统一压缩和复核。
